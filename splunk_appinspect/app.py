...
class App(object):
    ...
    def __init__(self, location):
        self.package_handler = app_package_handler.AppPackageHandler(location)
        ...
        if not platform.system() == "Windows":
            output = subprocess.check_output('find {} -print0 | xargs -0 file -L'.format(self.app_dir), shell=True)
            for line in output.split('\n'):
                c = line.split(':')
                if len(c) >= 2:
                    self.is_text_file[c[0]] = True if re.search(r':.* text', line, re.IGNORECASE) else False
    ...
    @property
    def package(self):
        """Helper function to return the main Splunk App's AppPackage object.

        Returns:
            AppPackage derived object
        """
        return self.package_handler.main_app_package
    ...
    def app_dir(self):
        ...
        Returns:
            String: an absolute path to the top level directory of the extracted
                Splunk App
        """
        return self.package.working_path
    ...
    def iterate_files(self, basedir='', excluded_dirs=None, types=None, excluded_types=None,
    ...
    def is_text(self, filename):
        ...
        if file_path in self.is_text_file:
            return self.is_text_file[file_path]
        try:
            cmd = ["file", file_path, "-L"]
